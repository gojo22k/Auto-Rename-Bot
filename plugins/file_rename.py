...
def extract_episode_number_from_caption(caption, format_template):
    # Extract episode number using regex patterns from the caption
    for pattern in [pattern1, pattern2, pattern3, pattern3_2, pattern4, patternX]:
        match = re.search(pattern, caption)
        if match:
            print(f"Matched Pattern in Caption: {pattern}")
...
async def auto_rename_files(client, message):
    ...
    # Extract information from the incoming media
    if message.document:
    ...
    # Extract episode number from the caption
    episode_number = extract_episode_number_from_caption(caption, format_template)
    ...
    if episode_number:
        ...
        caption = c_caption.format(filename=new_file_name, duration=convert(duration), size=humanbytes(os.path.getsize(file_path))) if c_caption else None

        try:
            if media_type == "video":
                ph_path = c_thumb if c_thumb else None
                await client.send_video(message.chat.id, path, caption=caption, thumb=ph_path, progress=progress_for_pyrogram, progress_args=("Uploading Video....", upload_msg, time.time()))
            elif media_type == "audio":
                ph_path = c_thumb if c_thumb else None
                await client.send_audio(message.chat.id, path, caption=caption, thumb=ph_path, progress=progress_for_pyrogram, progress_args=("Uploading Audio....", upload_msg, time.time()))
            elif media_type == "document":
                ph_path = c_thumb if c_thumb else None
                await client.send_document(message.chat.id, path, caption=caption, thumb=ph_path, progress=progress_for_pyrogram, progress_args=("Uploading Document....", upload_msg, time.time()))
        except Exception as e:
            await upload_msg.edit(f"Upload failed: {e}")
        finally:
            if os.path.isfile(path):
                os.remove(path)
            if ph_path and os.path.isfile(ph_path):
                os.remove(ph_path)
            # Remove the file from renaming operations after processing
            del renaming_operations[file_id]
